...
void AudioMixer_init(void)
{
	...
	AudioMixer_readWaveFileIntoMemory(STARTGAME_FILE, &startGame);
}
...
void AudioMixer_queueSound(wavedata_t *pSound)
{
	...
	bool soundBitesFull = true;

	for (int i = 0; i < MAX_SOUND_BITES; i++) {
		if (soundBites[i].pSound == NULL) {
			soundBitesFull = false;
...
void AudioMixer_cleanup(void)
{
	...
	AudioMixer_freeWaveFileData(&startGame);
...
static void fillPlaybackBuffer(short *buff, int size)
{
	...
	// 2. Since this is called from a background thread, and soundBites[] array may be used by any other thread, must synchronize this.
	pthread_mutex_lock(&audioMutex);

	// 3. Loop through each slot in soundBites[], which are sounds that are either waiting to be played, or partially already played. 
	for(int i = 0; i < MAX_SOUND_BITES; i++) {
		...
		if(pSound != NULL) {
			...
			for(int j = 0; j < size; j++) {
				if (offset < numSamples) {
					short sample = pSound->pData[offset];

					// "add" this sound bite's data to the play-back buffer.
					buff[j] += sample;
					
					// When adding values, ensure there is not an overflow.
					buff[j] = buff[j] > SHRT_MAX ? SHRT_MAX : buff[j];
					buff[j] = buff[j] < SHRT_MIN ? SHRT_MIN : buff[j];
					
					// Record that this portion of the sound bite has been played back by incrementing the location inside the data where play-back currently is.
					offset++;
				}
			}

			// Update the location of the sound bite.
...
void gameStartSound()
{
	AudioMixer_queueSound(&startGame);
	printf("Game start sound\n");
}
